...
% --- Construct directional basis Y_all: 3 per basis function (x, y, z directions) ---
Y_all = cell(1, 3 * M);
for i = 1:M
    Y_all{3*(i-1)+1} = [C(:, i), zeros(N, 1), zeros(N, 1)];  % x-dir
    Y_all{3*(i-1)+2} = [zeros(N, 1), C(:, i), zeros(N, 1)];  % y-dir
    Y_all{3*(i-1)+3} = [zeros(N, 1), zeros(N, 1), C(:, i)];  % z-dir
end
